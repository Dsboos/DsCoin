...
The default preset is _Medium_, which calculates in batches of 20,000 hashes. If you wish to increase or decrease the load on your system, you may select another preset accordingly from the preset drop down menu.
...
>
> This is what the `hash_info()` function for a `TxO` returns on calling.\
...
## CLASS: `TxO(sndr, rcvr, amt)`

> Fundamental unit of a `Tx`. It can be automatically created within a `Tx` by using the `.create_output()` method.
...
## CLASS: `Block(previous_hash, miner_pk)`
...
>
> The block contains an attribute known as the mine sequence (`mine_seq`), which is a part of the block's hash info. This sequence is incremented by one and hash of the block is computed. \
