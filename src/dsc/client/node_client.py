...
class NodeClient():
    ...
    async def fetch_chainstate(self):
        ...
        try:
            ...
            warn2(f"[Node Client] Couldn't fetch chainstate: couldn't connect to node!")
            return False, "Couldn't connect to node."

        #Requesting permission with header:
        writer.write(b"[chainstate_fetch_request]")
        await writer.drain()
        ...
        while True:
            chunk = await reader.read(1024)
            if not chunk:
                warn2(f"[Node Client] Couldn't fetch chainstate: Server closed connection!")
                return False, "Server closed connection."
            status += chunk
            if b"[get_ready]" in status:
                break
